...
const path = {
    ...
    require_conf: {
        src  : 'app/assets/js/require.config.js',
        dest : 'public/js/'
    },
    vendor: {
        src  : 'app/assets/js/vendor/*',
...
gulp.task( 'require_conf', () => {
    
    return gulp
        .src(path.require_conf.src)
        .pipe(gulp.dest(path.require_conf.dest))
        .pipe(connect.reload());
});
gulp.task( 'vendor', () => {
...
gulp.task('watch', () => {
    ...
    gulp.watch(path.images.src, ['require_conf']);
});
